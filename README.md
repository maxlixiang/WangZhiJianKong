...
- 支持手动触发一次巡检
- 支持查看所有监控商品的当前库存状态
- 支持通过 `/stop` 停止自动监控，并通过 `/start` 恢复
- 支持一键恢复所有已经停止的提醒
- 发现补货后向白名单用户发送 Telegram 消息
- 消息中带有停止提醒按钮，停止后不影响 `/status` 显示真实库存
...
启动或恢复自动监控。如果自动监控已经在运行，会返回当前已在运行的提示。

### `/help`

查看命令菜单。菜单会列出所有可用命令，包括 `/status`、`/restore` 和 `/stop`。
...
立即手动巡检所有监控任务，并对未停止提醒的有货商品发送 Telegram 提醒。
...
注意：如果某个商品已经点过“停止提醒”，`/check` 仍会把它统计为“当前有货”，但不会再次发送提醒。
...
`/status` 不会发送补货提醒，只用于查看真实状态。

### `/restore`

恢复所有已经停止的提醒。

执行后，`stock_state.json` 中所有 `acknowledged` 会被重置为 `false`。之后再运行 `/check`，如果商品仍然有货，机器人会重新发送提醒。

### `/stop`

停止自动定时监控。机器人进程仍会继续运行，之后可以继续使用 `/list`、`/status`、`/check` 等命令；需要恢复自动监控时运行 `/start`。
...
6. 如果没有“售罄”，则认为商品可购买。
7. 如果商品可购买且没有停止提醒，则发送 Telegram 提醒。

提醒状态保存在 `stock_state.json`。其中：

- `in_stock` 表示最近一次巡检得到的真实库存状态。
- `acknowledged` 表示用户是否已经点击过“停止提醒”。
